...
# Initialize MongoDB (add this right after other session_state checks)
if 'mongo_client' not in st.session_state:
    st.session_state.mongo_client = MongoClient("mongodb://localhost:27017/")  # Local MongoDB
    st.session_state.db = st.session_state.mongo_client["ai_visualizations"]
    st.session_state.prompts_collection = st.session_state.db["prompts"]
...
def chat_with_llm(e2b_code_interpreter: Sandbox, user_message: str, dataset_path: str) -> Tuple[Optional[List[Any]], str]:
    ...
    with st.spinner('Getting response from Together AI LLM model...'):
        ...
        if python_code:
            ...
            st.session_state.prompts_collection.insert_one({
...
def main():
    ...
    # ===== ADD MONGO DB INITIALIZATION HERE =====
    if 'mongo_client' not in st.session_state:
        try:
            st.session_state.mongo_client = MongoClient("mongodb://localhost:27017/")
            db = st.session_state.mongo_client["ai_visualizations"]
            collection = db["prompts"]
            
            # Test connection
            collection.insert_one({"status": "Connection test successful!", 
                                 "timestamp": datetime.now()})
            st.success("✅ Connected to MongoDB!")
        except Exception as e:
            st.error(f"❌ MongoDB connection failed: {e}")
            st.session_state.mongo_client = None
    # ===== END OF MONGO DB INIT =====
    ...
    if st.button("🕒 Show History"):
        history = list(st.session_state.prompts_collection.find().sort("timestamp", -1))  # Newest first
        if history:
            st.sidebar.subheader("Last 5 Prompts")
            for item in history[:5]:  # Show only recent 5
            # Defensive programming: ensure item is a dict-like object
                if not isinstance(item, dict):
                    st.sidebar.write("Invalid history entry (not a dict). Skipping...")
                    continue

            # Use .get with fallback strings
                user_prompt = item.get('user_prompt', 'No prompt found')
                generated_code = item.get('generated_code', 'No code found')

                st.sidebar.write(f"**Prompt:** {user_prompt}")
                st.sidebar.code(generated_code, language="python")
        else:
            st.sidebar.warning("No history yet!")
